    ...
    <el-form ref="form" :model="form" :rules="rules" label-width="80px">
      ...
      <el-form-item label="医院地区" prop="district">
        <el-select v-model="form.district" placeholder="请选择医院地区">
          <el-option v-for="(item,index) in districts.district" :key="item.name" :label="item.name" :value="item.name"></el-option>
        </el-select>
      </el-form-item>
...
export default {
  ...
  methods: {
    _getDetail() {
      this.imageUrl = this.$api.hospitalimg + 'default.jpg';
    ...
    onSubmit: function(form) {
      this.$refs[form].validate(valid => {
        if (valid) {
          ...
          axios.post(this.$api.hospitalnew, formData).then(
            response => {
              // success callback
