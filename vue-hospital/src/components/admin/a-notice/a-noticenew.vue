...
export default {
  data() {
    return {
      ...
      input: '',
      rules: {
        n_title: [{ required: true, message: '请输入标题', trigger: 'blur' }],
      },
    };
  },
  computed: {
  ...
  methods: {
    ...
    onSubmit: function(form) {
      this.$refs[form].validate(valid => {
        if (valid) {
          this.form.n_description = this.input;
          var formData = Qs.stringify(this.form);
          axios.post(this.$api.noticenew, formData).then(
            response => {
              // success callback
  ...
  created() {
    if (sessionStorage.getItem('admin')) {
      ...
      this.form.h_id = this.admin.h_id;
    }
  },
};
</script>
