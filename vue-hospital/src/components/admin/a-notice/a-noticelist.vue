    ...
    <el-table :data="tableData" class="updateTable">
      <el-table-column label="公告标题" width="180">
        <template slot-scope="scope">
          <span style="margin-left: 10px">{{ scope.row.n_title }}</span>
        </template>
      </el-table-column>
        <el-table-column label="内容">
        <template slot-scope="scope">
          <span style="margin-left: 10px" class="deDesc">{{ scope.row.n_description }}</span>
        </template>
      </el-table-column>
...
export default {
  ...
  methods: {
    handleEdit(index, row) {
      this.setNotice(row);
      this.$router.push({ path: '/admin/noticedetail' });
    },
    handleDelete(index, row, tableData) {
      axios.delete(this.$api.noticedelete + `/${row.n_id}`).then(
        response => {
          // success callback
    ...
    handleCurrentChange(val) {
      axios.get(this.$api.getnoticelist + `/${this.hospitalId}/${val}`).then(
        response => {
          // success callback
          console.log(response);
  ...
  created() {
    if (sessionStorage.getItem('admin')) {
      ...
      axios.get(this.$api.getnoticelist + `/${this.hospitalId}/${this.currentPage}`).then(
      response => {
        // success callback
        console.log(response);
        this.dataTotal = response.data.total;
